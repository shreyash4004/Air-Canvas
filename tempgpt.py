            ...
            if drawing:
                ...
                if len(cnts) > 0:
                    ...
                    if identified_shape != "Unknown":
                        ...
                        if identified_shape == "Circle":
                            perfect_shape = cv2.circle(np.zeros_like(frame), (int(x), int(y)), int(radius), (255, 255, 255), -1)
                        elif identified_shape == "Square":
                            perfect_shape = cv2.rectangle(np.zeros_like(frame), (int(x-radius), int(y-radius)), (int(x+radius), int(y+radius)), (255, 255, 255), -1)
                        elif identified_shape == "Rectangle":
                            perfect_shape = cv2.rectangle(np.zeros_like(frame), (int(x-radius), int(y-radius)), (int(x+radius), int(y+radius*2)), (255, 255, 255), -1)

                        perfect_shape = cv2.resize(perfect_shape, (frame.shape[1], frame.shape[0]))
                        frame = cv2.addWeighted(frame, 1, perfect_shape, 1, 0)
                    ...
                    if center[1] <= 65:
                        if 40 <= center[0] <= 140:  # Clear Button
                            ...
                            if 1 <= center[1] <= 33:  # SAVE button
                                cv2.imwrite("canvas_image.png", frame[67:, :, :])
                            elif 40 <= center[1] <= 71:  # REOPEN button
                                if os.path.exists("canvas_image.png"):
                                    frame[67:, :, :] = cv2.imread("canvas_image.png")
                    else:
                        if colorIndex == 0:
